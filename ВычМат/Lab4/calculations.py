...
import numpy as np
    ...
    #R^2
    # mid_phi = [(1 / n * result_func(points[i][0])) for i in range(n)]
    # err = [(points[i][1] - mid_phi[i]) ** 2 for i in range(n)]
    # print(1 - (sum(errors)) / (sum(err)))
    #R^2 >= 0.95 - высокая
    #0.95 > R^2 >= 0.75 - удовлетворительно
    #0.75 > R^2 >= 0.5 - слабая
    #
...
def r_check(k_det):
    k_det = round(k_det, 3)
    if k_det >= 0.95:
        return f"{k_det} >= 0.95 - высокая точность апроксимации"
    elif k_det >= 0.75 and k_det < 0.95:
        return f"0.95 > {k_det} >= 0.75 - удовлетворительная точность апроксимации"
    elif k_det >= 0.5 and k_det < 0.75:
        return f"0.75 > {k_det} >= 0.5 - низкая точность апроксимации"
    else:
        return f"{k_det} < 0.5 - недостаточная точность апроксимации"
...
def linear_approximate(points):
    ...
    mid_phi = [(1 / n * result_func(points[i][0])) for i in range(n)]
    err = [(points[i][1] - mid_phi[i]) ** 2 for i in range(n)]
    k_det = (1 - (sum(errors)) / (sum(err)))

    return result_func, str_result_func, errors, mid_sqd_err, k_det
