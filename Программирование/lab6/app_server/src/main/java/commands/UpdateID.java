...
public class UpdateID extends Command {
    ...
    public UpdateID(JsonArray parameters, JsonArray arguments) {
    ...
    public UpdateID(String[] parameters, PrintStream stdout, Scanner scanner) throws ArgumentException{
        if (parameters.length != 0) throw new ArgumentException("Данная команда не принимает аргументы");
        this.id = ArgumentReader.readLong(stdout, scanner, "Введите ID, который хотите изменить");
    ...
    public String execute(MyTreeSet treeSet) {
        ...
        City newcity = new City(id, name, coordinates, area, population, metersAboveSeaLevel, climate, government, standardOfLiving, governor);
        treeSet.getTreeSet().add(newcity);

        return "Элемент изменён";
    }

    public static Help.HelpEntry getHelpEntry() {
        return new Help.HelpEntry("update_id", "обновляет значение элемента коллекции, id которого равен заданному");
    }
}
